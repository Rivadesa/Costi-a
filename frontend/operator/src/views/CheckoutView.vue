...
function tableName(tableId) {
  return tableMap.value.get(tableId)?.name || 'Mesa';
}
...
async function load() {
  ...
  if (!selectedServiceId.value && rows.value.length) {
...
async function recordPayment() {
  ...
  await act(() => api.checkoutAddPayment(selectedServiceId.value, {
...
async function closeService() {
  await act(() => api.checkoutClose(selectedServiceId.value));
}
</script>
    ...
    <div class="checkout-layout">
      <aside class="panel checkout-service-list">
        ...
        <button
          v-for="row in rows"
          :key="row.service_id"
          class="checkout-service-button"
          :data-active="row.service_id === selectedServiceId"
          @click="selectService(row.service_id)"
        >
      ...
      <main v-if="account" class="stack gap-lg">
        <section class="panel">
          ...
          <div class="checkout-lines">
            <div v-if="account.menu" class="account-row">
              <span>{{ account.menu.quantity }} × {{ account.menu.name }}</span>
              <strong>{{ money(account.menu.total_cents) }}</strong>
            </div>
            <div v-for="item in account.consumptions.filter(item => !item.cancelled)" :key="item.id" class="account-row">
              ...
              <div class="account-row-actions">
                ...
                <button v-if="hasPermission('consumption.cancel')" class="link-button" @click="cancelConsumption(item)">Anular</button>
              </div>
            </div>
          </div>
        </section>
          ...
          <form class="checkout-add-form" @submit.prevent="addProduct">
            <label>Artículo
              <select v-model="selectedProductId" required>
                ...
                <option v-for="item in saleItems" :key="item.id" :value="item.id">{{ item.name }}<template v-if="item.format_label"> · {{ item.format_label }}</template> · {{ money(item.price_cents) }}</option>
              </select>
            </label>
        ...
        <section v-if="hasPermission('payment.record')" class="panel">
          <div class="panel-heading">
            <div><p class="eyebrow">Cobro</p><h2>Pendiente</h2></div>
            <div class="account-total">{{ money(account.balance_cents) }}</div>
          </div>
