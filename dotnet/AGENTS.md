...
D1.3: read ADR-010 and docs/native/D1.3-native-trial.md. A per-user Windows trial installer/supervisor is implemented under Costina.TrialHost/Costina.Launcher; not SCM or production provisioning. It must not touch legacy data, expose LAN, reset data, grant runtime DDL/delete, log secrets, or treat lab roles as production device identity. Installer tests use native PostgreSQL on Windows. Update evidence, not just claims.
