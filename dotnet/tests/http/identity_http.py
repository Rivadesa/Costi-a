...
class IdentityChecks(unittest.TestCase):
    ...
    def test_05_sliding_renewal_extends_but_absolute_cap_wins(self):
        ...
        before = h.sql("SELECT max(expires_at) FROM core.sessions WHERE tenant='d4-identity'")
        h.sql("UPDATE core.sessions SET expires_at = expires_at - interval '10 minutes' WHERE tenant='d4-identity'")
        self.assertEqual(200, bearer('/session', data['token'])[0])
        after = h.sql("SELECT max(expires_at) FROM core.sessions WHERE tenant='d4-identity'")
        self.assertGreaterEqual(after, before)
        h.sql("UPDATE core.sessions SET absolute_expires_at = now() - interval '1 second' WHERE tenant='d4-identity'")
        self.assertEqual(401, bearer('/session', data['token'])[0])
        h.sql("UPDATE core.sessions SET absolute_expires_at = now() + interval '1 hour' WHERE tenant='d4-identity'")

    def test_06_expiry_logout_and_deactivation_revoke_access(self):
        _, data = login('maitre', PASSWORD)
        h.sql("UPDATE core.sessions SET expires_at = now() - interval '1 second' WHERE tenant='d4-identity'")
        self.assertEqual(401, bearer('/session', data['token'])[0])
        ...
        h.sql("UPDATE core.users SET active=false WHERE tenant='d4-identity' AND username='maitre'")
        self.assertEqual(401, bearer('/session', again['token'])[0])
        h.sql("UPDATE core.users SET active=true WHERE tenant='d4-identity' AND username='maitre'")

    def test_07_only_hashes_at_rest(self):
        _, data = login('cocinero', PASSWORD)
        stored = h.sql("SELECT string_agg(token_hash, ',') FROM core.sessions WHERE tenant='d4-identity'")
        self.assertNotIn(data['token'], stored)
        self.assertTrue(all(len(x) == 64 for x in stored.split(',')))
        hashes = h.sql("SELECT string_agg(password_hash, ',') FROM core.users WHERE tenant='d4-identity'")
