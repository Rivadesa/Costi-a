...
// Solo toca las tablas de SU esquema (dining.services, dining.occupancies). La cuenta (core.accounts) es del nucleo y se pide a la Unit.
public static class DiningUnit
{
    public static async Task<StoredDining> Dining(this Unit unit, string id)
    {
        var rows = await unit.Rows("SELECT version,state,payload::text,payload_version FROM dining.services WHERE " + Unit.ScopeWhere + " AND id=@id",
    ...
    {
        var rows = await unit.Rows("SELECT id,version,state,payload::text,payload_version,table_id FROM dining.occupancies WHERE " + Unit.ScopeWhere + " AND service_id=@id",
    ...
    public static Task Save(this Unit unit, DiningService entity, long? expectedVersion)
        => unit.SaveAggregate("dining.services", entity, entity.State.ToString(), Wire.Encode(entity.Snapshot()), expectedVersion, "table_id", entity.TableId);
    ...
    public static async Task Save(this Unit unit, TableOccupancy entity, long? expectedVersion)
    {
        if (expectedVersion is null)
        {
            ...
            await unit.Sql("INSERT INTO dining.occupancies (tenant,company,location,id,service_id,table_id,state,version,payload) VALUES (@tenant,@company,@location,@id,@service,@table,@state,1,@payload::jsonb)",
        ...
        else await unit.SaveAggregate("dining.occupancies", entity, entity.State.ToString(), Wire.Encode(entity.Snapshot()), expectedVersion, "service_id", entity.ServiceId);
    }

    public static async Task<IReadOnlyList<BoardRow>> Board(this Unit unit)
    {
        var ids = await unit.Rows("SELECT service_id FROM dining.occupancies WHERE " + Unit.ScopeWhere + " AND state='Occupied' ORDER BY table_id LIMIT 200", [], r => r.GetString(0));
