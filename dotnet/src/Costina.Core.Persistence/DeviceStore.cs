...
public sealed class DeviceStore(NpgsqlDataSource dataSource, BusinessScope scope)
{
    ...
    public async Task<PairingIssued> CreatePairingAsync(string createdBy, CancellationToken ct = default)
    {
        ...
        await using var command = Command(
            "INSERT INTO native_d1.pairings (tenant,company,location,id,code_hash,created_by,expires_at) " +
    ...
    public async Task<(string PairingId, string PollSecret)?> ClaimAsync(string code, string deviceName, CancellationToken ct = default)
    {
        ...
        await using var command = Command(
            "UPDATE native_d1.pairings SET status='claimed', device_name=@name, poll_secret_hash=@poll, claimed_at=now() " +
    ...
    {
        await using var command = Command(
            "SELECT id, device_name, claimed_at FROM native_d1.pairings WHERE " + ScopeWhere +
        ...
        string decidedBy, CancellationToken ct = default)
    {
        ...
        await using var command = Command(
            "UPDATE native_d1.pairings SET status=@status, approved_role=@role, approved_station=@station, decided_by=@by " +
        ...
        string pairingId, string pollSecret, CancellationToken ct = default)
    {
        await using (var consume = Command(
            "UPDATE native_d1.pairings SET status='consumed' WHERE " + ScopeWhere +
            " AND id=@id AND poll_secret_hash=@poll AND status='approved' RETURNING device_name, approved_role, approved_station, decided_by"))
        {
            ...
            if (await reader.ReadAsync(ct))
            {
                ...
                await using var insert = Command(
                    "INSERT INTO native_d1.devices (tenant,company,location,id,name,secret_hash,role,station,approved_by) " +
        ...
        await using var peek = Command(
            "SELECT status FROM native_d1.pairings WHERE " + ScopeWhere + " AND id=@id AND poll_secret_hash=@poll");
    ...
    {
        await using var command = Command(
            "SELECT name, role, station FROM native_d1.devices WHERE " + ScopeWhere +
    ...
    {
        await using var command = Command(
            "SELECT id, name, role, station, approved_by, created_at, revoked_at FROM native_d1.devices WHERE " +
    ...
    {
        await using var command = Command(
            "UPDATE native_d1.devices SET revoked_at=now(), revoked_by=@by WHERE " + ScopeWhere +
