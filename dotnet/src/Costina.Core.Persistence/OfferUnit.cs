...
public sealed record OfferCourseView(string Id, string Name, int Sort, bool Active, OfferDishDefinition[] Dishes);
...
public static class OfferUnit
{
    ...
    // Oferta con la que se puede ABRIR una mesa ahora: vigente y con al menos un pase activo con un plato activo. Devuelve
    // sus pases activos con sus platos activos, en orden.
    public static async Task<(OfferDefinition Offer, OfferCourseView[] Courses)> AvailableOffer(this Unit unit, string id, DateTime at)
    {
        ...
        var courses = (await unit.OfferCourses(id)).Where(c => c.Active).Select(c => new OfferCourseView(c.Id, c.Name, c.Sort, c.Active, dishes[c.Id].ToArray()))
            .Where(c => c.Dishes.Length > 0).ToArray();
        if (courses.Length == 0) throw new RuleViolation("offer_incomplete", "This offer has no course with dishes yet.");
...
public static class OfferReads
{
    ...
    public static async Task<OfferView[]> Offers(this DesktopReadRepository reads, BusinessScope scope, CancellationToken ct)
    {
        ...
        return offers.Select(o => new OfferView(o.Id, o.Name, OfferUnit.KindText(o.Kind), o.ProductId, OfferUnit.ServiceText(o.Service), o.ValidFrom?.ToString("yyyy-MM-dd"), o.ValidTo?.ToString("yyyy-MM-dd"), o.Weekdays, o.Sort, o.Active,
            courses.Where(c => c.OfferId == o.Id).Select(c => new OfferCourseView(c.Id, c.Name, c.Sort, c.Active, dishes[(o.Id, c.Id)].ToArray())).ToArray())).ToArray();
    }
