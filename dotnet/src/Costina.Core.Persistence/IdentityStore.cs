...
public sealed class IdentityStore(NpgsqlDataSource dataSource, BusinessScope scope)
{
    ...
    public async Task<string> CreateUserAsync(string username, string password, string role, CancellationToken ct = default)
    {
        ...
        await using var command = Command(
            "INSERT INTO core.users (tenant,company,location,id,username,password_hash,role) " +
        ...
        string username, string password, CancellationToken ct = default)
    {
        ...
        await using var find = Command(
            "SELECT id, password_hash, role FROM core.users WHERE " + ScopeWhere + " AND username=@username AND active");
        ...
        await using var insert = Command(
            "INSERT INTO core.sessions (tenant,company,location,id,user_id,token_hash,expires_at,absolute_expires_at) " +
    ...
    {
        await using var command = Command(
            "UPDATE core.sessions s SET expires_at = LEAST(now() + @sliding, s.absolute_expires_at) " +
            "FROM core.users u WHERE s.tenant=@tenant AND s.company=@company AND s.location=@location" +
    ...
    {
        await using var command = Command(
            "UPDATE core.sessions SET revoked_at = now() WHERE " + ScopeWhere + " AND id=@id AND revoked_at IS NULL");
    ...
    {
        await using var command = Command("SELECT count(*) FROM core.users WHERE " + ScopeWhere);
