...
- Los commits posteriores de esta rama solo añaden este traspaso y `tools/agent/`: **el código es el de `f6a783b`**. Ojo: el filtro `paths` de `pull_request` evalúa el diff de TODA la PR, así que cada push relanza los cuatro workflows aunque solo toque documentación. **Leer el CI de la punta de la rama** (`git rev-parse --short origin/feat/d5.5-installer`); al cerrar esta sesión seguía en curso.
...
Primera tarea: la PR #48 (D5.5, instalador; rama feat/d5.5-installer) está abierta. Lee GitHub Actions de la PUNTA de la rama (sh tools/agent/wait-ci.sh feat/d5.5-installer $(git rev-parse --short origin/feat/d5.5-installer) y después los LOGS de los jobs; el código es el de f6a783b, los commits posteriores son solo el traspaso), corrige lo que salga en el workflow "Native Windows installer" y, con mi confirmación, mézclala con merge commit "Merge PR #48: D5.5 instalador de Windows". installer_checks.ps1 re-ejecuta setup-server como diagnóstico cuando la instalación silenciosa falla: esa salida está en el log del job.
