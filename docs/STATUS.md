...
- D6.2 (issue #28, corte 2): comandero de solo lectura en la PWA — tablero de mesas y detalle con restricciones por comensal (tipo, sustancia y severidad siempre escritos) y revisión pendiente de cocina —, tiempo real con el billete efímero del hub (uno nuevo por conexión y reconexión; el token del dispositivo nunca viaja en una URL), relectura autoritativa en cada aviso y obligatoria tras reconectar, degradación visible a sondeo y antigüedad de datos siempre a la vista. El motor no cambia. Ver `docs/native/D6.2-pwa-waiter-read.md` y el CI de su PR.

- D6.3 (issue #28, corte 3): acciones del comandero en la PWA (abrir, iniciar, enviar, servir, omitir, pausar/reanudar, restricciones y consumos sin precios), habilitadas solo por las affordances del servidor, con **una orden incierta durable en IndexedDB** — guardada antes del primer intento, reintento con bytes y clave idénticos, cierre solo con éxito con eco o rechazo definitivo, conciliación por clave, nunca reenvío de órdenes ajenas o ilegibles —. Probado con respuestas perdidas reales y cortes de red contra el motor: un solo efecto. Único cambio del motor: la respuesta de `add-consumption` devuelve `consumptionId` (antes `chargeId`, que la guarda anti-dinero de la PWA rechazaba con razón). Ver `docs/native/D6.3-pwa-durable-command.md` y el CI de su PR.

- D6.4 (issue #28, corte 4): pantalla de cocina por estación en la PWA — cada estación ve el pase entero con lo ajeno atenuado y sin botones, marca solo lo suyo, y el `pase` valida y decide las revisiones de restricciones (no afecta / adaptar / rehacer, con nota obligatoria) —, con objetivos táctiles grandes, restricciones siempre escritas con severidad y todo habilitado solo por las affordances del servidor; misma lectura en vivo y misma orden incierta que el comandero (`operations.ts`). Único cambio del motor: `review-preparation` deja de anunciarse a estaciones que no son `pase` (el comando ya lo rechazaba). Ver `docs/native/D6.4-pwa-kds.md` y el CI de su PR.

- D6.5 (issue #28, corte 5 y último): QR de emparejamiento en la pestaña Puestos del WPF (`https://EQUIPO:PUERTO/app/#pair=CÓDIGO`, solo el código de un uso y en el fragmento; lo lee la cámara del dispositivo, sin escáner propio), PWA instalable en Android e iOS (iconos PNG 192/512, maskable y `apple-touch-icon`) y **guion físico con Android e iPad** con tabla de resultados por criterio (`docs/native/D6.5-qr-install-physical.md`). Ver el CI de su PR.

- E1a (Hito 6, núcleo del ERP, ADR-012): el código se reorganiza en núcleo (`Costina.Core.*`) y módulos (`Costina.Modules.Dining*`) con contrato de módulo (`IModule`), rutas de Dining bajo `/dining/…` con alias de compatibilidad, `session.modules` y superficies de los clientes montadas por módulo. Sin cambio de comportamiento ni de esquema. Ver `docs/native/E1a-modular-code.md`.
- E1b (Hito 6, ADR-012): esquema versión 2 = `core` (antes `native_d1`) más un esquema por módulo (`dining`, con `schema.sql` y `grants.sql` propios). `upgrade` migra v1→v2 en una transacción; la cuenta guarda la mesa y deja de depender del módulo; menús en `dining.configuration`; módulos activos en `core.installation.modules`; copias de todos los esquemas con manifiesto formato 2 y **copias v1 aún restaurables** (verificadas con su manifiesto y migradas después). Probado con una base v1 real construida desde fixtures congelados. Ver `docs/native/E1b-schema-per-module.md`.
- E2 (Hito 6): organización editable del núcleo con pantalla: salas, mesas y estaciones relacionales (`core.zones/tables/stations`, esquema v3 con migraciones encadenadas), `GET /organization` y comandos `organization/commands/{action}` (solo principal, auditados), nada se borra (se desactiva), mesa en uso no desactivable por contrato del módulo, estación `pase` reservada, aprobación de puestos con estación activa; pestaña **Configuración** en el WPF y **tema visual** propio (verde bosque, crema, cobre) aplicado a todas las pestañas. Ver `docs/native/E2-organization.md`.
...
- Coste de CI (21-09, sin cambio de producto ni de versión): cada workflow nativo cancela el run anterior de la misma PR e ignora cambios solo de Markdown; el instalador de Windows deja de correr en cada push de PR (solo si cambia lo que empaqueta o invoca), corre siempre al mezclar en `develop` y a petición. `tools/agent/local-tests.sh` localiza el SDK también fuera de AppData (Python de Microsoft Store). Reglas en `dotnet/AGENTS.md`.

Hito 3 (#26): completo en código con D4.3b (PR #43); **su cierre espera el guion manual del promotor**. Hito 4 (#27): **completo en código** con D5.1–D5.6 (PR #44–#48 y la de D5.6); **su cierre espera el guion físico de D5.6 ejecutado por el promotor** y la firma queda en #12. Hito 5 (#28): **completo en código** con D6.1–D6.5 (PR #50–#53 y la de D6.5); **su cierre espera el guion físico de D6.5 (Android e iPad) ejecutado por el promotor**.
