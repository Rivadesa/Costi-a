...
final class CheckoutCatalogApiTest extends TestCase
{
    public function test_service_tracking_hides_money_while_checkout_snapshots_catalog_price(): void
    {
        ...
        config([
            'hospitality.tenant_id' => RetiroDemoSeeder::TENANT_ID,
        ...
        $opened = $this->withToken($token)
            ->withHeader('Idempotency-Key', 'checkout-open-'.Ulid::generate())
            ->postJson('/api/v1/services', [
                'table_id' => $tableId,
                'pax' => 2,
                'menu_id' => RetiroDemoSeeder::MENU_ID,
            ])
            ->assertCreated();

        $serviceId = (string) $opened->json('service_id');

        $operational = $this->withToken($token)
            ->getJson('/api/v1/services/'.$serviceId)
            ->assertOk()
            ->json();
        ...
        $added = $this->withToken($token)
            ->withHeader('Idempotency-Key', 'checkout-item-'.Ulid::generate())
            ->postJson('/api/v1/checkout/services/'.$serviceId.'/consumptions', [
            ...
            ->assertJsonPath('unit_price_cents', 400)
        ...
        DB::table('product_prices')
            ...
            ->update(['price_cents' => 999, 'updated_at' => now()]);

        $checkout = $this->withToken($token)
            ->getJson('/api/v1/checkout/services/'.$serviceId)
            ->assertOk()
            ->assertJsonPath('menu.unit_price_cents', 15000)
            ->assertJsonPath('consumptions.0.product_id', $productId)
            ->assertJsonPath('consumptions.0.unit_price_cents', 400)
            ->assertJsonPath('consumptions.0.total_cents', 800)
            ->assertJsonPath('subtotal_cents', 30800);
        ...
        $boardRow = collect($this->withToken($token)->getJson('/api/v1/service-board')->assertOk()->json('data'))
